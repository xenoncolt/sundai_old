  ...
  run: async (client, message, args, cmduser, text, prefix) => {
    ...
    try {
      if (args[0]) {
        ...
        let discord_server = new MessageButton().setStyle('LINK').setEmoji("960452826289545246").setLabel("Server").setURL("https://discord.gg/qxNqZ6vfhR")
        let menuOptions = [
          {
            label: "Overview",
            value: "Overview",
        ...
        let buttonRow = new MessageActionRow().addComponents([button_back, button_home, button_forward, discord_server])
        ...
        let OverviewEmbed = new MessageEmbed()
          ...
          .addField(":muscle: **__My Features__**",
            `>>> **58+ Systems**, 
            **Like:** <:twitter:960454253653139476> **Twitter-** & <:YTLogo:916792583282368593> **Youtube-Auto-Poster** 
**Application-**, Ticket-, **Welcome-Images-** and Reaction Role-, ... Systems
:notes: An advanced <:spotify:960455226018635806> **Music System** with **Audio Filtering**
...
📶 **\`${Math.floor(client.ws.ping)}ms\` Ping**`)
        ...
        collector.on('end', collected => {
          ...
          let d_buttonRow = new MessageActionRow().addComponents([button_back.setDisabled(true), button_home.setDisabled(true), button_forward.setDisabled(true), discord_server])
      ...
      function allotherembeds_eachcategory(filterdisabled = false) {
        ...
        var embed0 = new MessageEmbed()
          ...
          .addField(`<:discord:960454218894934016> **Server Related Commands**`, ">>> " + client.commands.filter((cmd) => cmd.category === "🔰 Info" && cmd.type === "server").sort((a, b) => a.name.localeCompare(b?.name)).map((cmd) => `\`${cmd.name}\``).join("︲"))
        ...
        var embed6 = new MessageEmbed()
          ...
          .addField("\u200b", "__**Sub-Categorized Commands:**__")
          .addField("<:discord:960454218894934016> **Server Related Commands**", "> " + client.commands.filter((cmd) => cmd.category === "🚫 Administration" && cmd.type.includes("server")).sort((a, b) => a.name.localeCompare(b?.name)).map((cmd) => `\`${cmd.name}\``).join("︲"))
        ...
        var embed9 = new MessageEmbed()
          ...
          .addField("\u200b", "__**Sub-Categorized Commands:**__")
          .addField("<:discord:960454218894934016> **Information & Manage**", "> " + client.commands.filter((cmd) => cmd.category === "👑 Owner" && cmd.type.includes("info")).sort((a, b) => a.name.localeCompare(b?.name)).map((cmd) => `\`${cmd.name}\``).join("︲"))
